...
【优化严格要求】
1. **纯粹的 Markdown 格式输出**：不要说任何客套话，不要包含诸如 ```markdown 的代码块前后缀包裹，直接开始输出文档内容。使用清晰的结构（例如：个人基本信息、求职意向、专业技能、工作经验/项目经验、教育背景）。
2. **STAR法则精修**：对用户的每一段经历使用STAR法则（情境、任务、行动、结果）进行重写。
3. **增加技术强动词与量化结果**：例如将“做了个功能”改为“主导设计并实现了XXX模块，系统吞吐量提升了20%，有效降低了服务器负载”。
4. 目标岗位参考：{target_job}
