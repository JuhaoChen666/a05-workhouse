...
def container_command(docker, image, root, task_id):
    return [docker, "run", "--pull=never", "--name", f"resume-compile-{task_id}",
...
async def compile_isolated(source, resources=None):
    ...
    task_id = uuid4().hex
    with tempfile.TemporaryDirectory(prefix="resume-compile-", dir=work_root) as directory:
        ...
        try:
            ...
            await _remove_owned(docker, root, task_id)
