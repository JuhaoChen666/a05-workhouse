"""Frozen initial four-table schema; Refs #1. No mutable model imports."""
from alembic import op, context
import sqlalchemy as sa
from sqlalchemy.dialects.mysql import DATETIME
revision = "phase1_storage_001"
down_revision = None
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    resume_type, optimization_type = sa.BigInteger(), sa.String(36)
    if not context.is_offline_mode():
        version = op.get_bind().scalar(sa.text("SELECT VERSION()"))
        import re
        match = re.match(r"(\d+)\.(\d+)\.(\d+)", version)
        if not match or "MariaDB" in version or tuple(map(int, match.groups())) < (8, 0, 16):
            raise RuntimeError("MySQL 8.0.16+ with enforced CHECK required")
        inspector = sa.inspect(op.get_bind())
        for name, key in (("resumes", "id"), ("resume_optimizations", "session_id")):
            columns = {c["name"]: c for c in inspector.get_columns(name)}
            if key not in columns or "user_id" not in columns or not isinstance(columns["user_id"]["type"], sa.Integer):
                raise RuntimeError("incompatible legacy owner/key structure")
            if inspector.get_pk_constraint(name)["constrained_columns"] != [key]:
                raise RuntimeError("legacy reference must be the expected primary key")
            if name == "resumes":
                resume_type = columns[key]["type"]
                if not isinstance(resume_type, sa.BigInteger):
                    raise RuntimeError("expected legacy BIGINT id")
            else:
                optimization_type = columns[key]["type"]
                if not isinstance(optimization_type, sa.String) or optimization_type.length != 36:
                    raise RuntimeError("expected legacy VARCHAR(36) optimization key")
                optimization_type = sa.String(36, collation=columns[key]["type"].collation or inspector.get_table_options(name).get("mysql_collate"))
    sa.Table("resumes", metadata, sa.Column("id", resume_type, primary_key=True))
    sa.Table("resume_optimizations", metadata, sa.Column("session_id", optimization_type, primary_key=True))
    tables = define_tables(metadata, resume_type, optimization_type)
    for table in tables:
