"""Frozen P2 import schema; Refs #1. No application model imports."""
...
revision = 'phase2_experience_001'
down_revision = 'phase1_storage_002'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    legacy_type = sa.BigInteger()
    if not context.is_offline_mode():
        columns = {c['name']: c for c in sa.inspect(op.get_bind()).get_columns('resumes')}
        legacy_type = columns['id']['type']
    sa.Table('resumes', metadata, sa.Column('id', legacy_type, primary_key=True))
    for table in define_import_tables(metadata, legacy_type):
